...
export default function ModalGroup({userId}: {userId: string;}) {
  ...
  const [ state, setState ] = useState<ResponseGroupAction>(initialState)
      ...
      <Box className="bg-gray-800 w-full max-w-[400px] min-h-fit max-h-[600px] rounded px-4 py-8 outline-none">
        ...
        <form className="flex flex-col gap-y-4 mt-5" onSubmit={bindSubmit}>
          ...
          <button
            ...
            className={`${
              !pending ? "bg-gray-600 hover:bg-gray-700" : "bg-gray-400 cursor-default"
          ...
          {pending && (
            <div className="flex justify-center items-center p-2">
              <span className="w-[40px] h-[40px] border-4 rounded-full animate-spin border-t-blue-400"></span>
            </div>
          )}
          {state.message && (
            <p
